...
static void e4c_propagate(void)
{
    ...
    if (fprintf(stderr, e4c.err.file ? err_msg[1] : err_msg[0],
                e4c.err.type->name, e4c.err.message, e4c.err.file,
                e4c.err.line) > 0) {
        (void) fflush(stderr);
    }
...
int e4c_try(const char *file, int line)
{
    if (e4c.frames >= E4C_MAX_FRAMES) {
        e4c_throw(&RuntimeException, file, line,
               ...
               const char *message)
{
    ...
    (void) sprintf(e4c.err.message, "%.*s", (int) E4C_MESSAGE_SIZE - 1,
                   (message ? message : e4c.err.type->default_message));
